...
function MobilePricePredictor() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post('https://622nmp2sw8.execute-api.ap-southeast-2.amazonaws.com/test/predictmobileprice', {
  ...
  return (
    <Container maxWidth="md">
      ...
      <Box mt={4}>
        {priceRange ? (
          <Typography variant="h6">Predicted Price Range: {priceRange}</Typography>
        ) : (
          <Typography variant="h6">Please input the details and click "Predict Price Range"</Typography>
        )}
      </Box>
    </Container>
  );
